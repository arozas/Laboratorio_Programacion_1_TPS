...
{
	int rtn = -1;
	Node* bufferNewNode = NULL;
	Node* bufferPreviousNode = NULL;

	bufferNewNode = (Node*)malloc(sizeof(Node));

	if(this != NULL && nodeIndex >= 0 && bufferNewNode != NULL)
	{
		//Como no se donde va estar el nuevo nodo.
		//¿Primera posició?¿Siguiente?¿última?
		//Por eso en el buffer salvo la referencia al index
		bufferNewNode->pElement = pElement;
		if(nodeIndex == 0)
		{
			bufferNewNode->pNextNode = this->pFirstNode;
			this->pFirstNode = bufferNewNode;
			rtn = 0;
		}
		else
		{
			...
			bufferPreviousNode = getNode(this, nodeIndex-1);
			if(bufferPreviousNode!= NULL)
			{
				bufferNewNode->pNextNode = bufferPreviousNode->pNextNode;
				bufferPreviousNode->pNextNode= bufferNewNode;
				rtn = 0;
			}
		}
		if(rtn == 0)
...
{	//retorno -1 si la lista
	...
	{	//recorro la lista,
		...
		{	//Si re es distinto a 0, retorno -2, si el puntero es null o menor a 0.
